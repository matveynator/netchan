# “Quantum” Network Channels in Go — Protocol v5
...
В протоколе v5 сетевой канал умеет передавать значения, reply channels,
...
v5. При этом NetChan не скрывает физику распределённой системы: сеть требует
...
> **Статус проекта:** protocol v5 готов для тестирования реальными
...
Между компьютерами общего runtime нет. В NetChan v5 роль network hypervisor
...
| Ограничение protocol v1 | Реализация в protocol v5 |
|---|---|
...
## Какие свойства каналов Go реализованы в v5
...
даже если физический TCP/TLS transport временно заменяется. NetChan v5:
...
У protocol v5 больше нет функциональных ограничений, перечисленных для v1.
...
Protocol v5 использует собственный ограниченный формат на основе
...
Лимиты v5 не являются недостающими возможностями. Они не позволяют
...
Wire protocol v5 и generic API намеренно несовместимы со старыми method-based
версиями NetChan. Обе стороны одного соединения должны использовать protocol
v5 и совпадающие схемы сообщений.
